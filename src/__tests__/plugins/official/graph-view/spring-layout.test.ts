...
describe('runSpringLayout', () => {
  ...
  it('places single node near center', () => {
    ...
    expect(Math.abs(pos.x - 400)).toBeLessThan(100); // widened from 50
    expect(Math.abs(pos.y - 300)).toBeLessThan(100);
  });
  ...
  it('warns when node count exceeds threshold', () => {
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      expect.stringContaining('101'),  // 101 nodes > threshold of 100
    );
