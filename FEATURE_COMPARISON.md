# MarkLite 功能对比

> 最后更新：2026-04-13 | MarkLite v0.7.1 (当前版本)

本文将 MarkLite 与市场主流 Markdown 编辑器进行功能横向对比，帮助用户了解各工具的适用场景。

---

## 对比工具

| 工具 | 类型 | 授权 | 价格 |
|------|------|------|------|
| **MarkLite v0.7.1** | 桌面 Markdown 编辑器 | MIT 开源 | 免费 |
| Typora | 桌面 WYSIWYG 编辑器 | 闭源 | ¥89（一次性）|
| Obsidian | 知识管理 / PKM 工具 | 闭源核心+插件生态 | 免费（个人）|
| VS Code + 插件 | 通用代码编辑器 | MIT 开源 | 免费 |
| Mark Text | 桌面 Markdown 编辑器 | MIT 开源 | 免费 |

---

## 核心编辑功能

| 功能 | MarkLite | Typora | Obsidian | VS Code | Mark Text |
|------|----------|--------|----------|---------|-----------|
| 实时分栏预览 | ✅ | ✅（单视图） | ✅（分屏） | ✅（插件） | ✅ |
| 多标签页编辑 | ✅ | ❌ | ✅ | ✅ | ❌ |
| 标签页拖拽排序 | ✅ | ❌ | ❌ | ✅ | ❌ |
| 自动保存 | ✅（1s 防抖） | ✅ | ✅ | ✅（插件） | ✅ |
| 自动括号补全 | ✅ | ✅ | ✅ | ✅ | ✅ |
| Vim 键盘模式 | ✅ | ❌ | ✅（插件） | ✅（插件） | ❌ |
| 多光标编辑 | ✅ | ❌ | ❌ | ✅ | ❌ |
| 代码语法高亮 | ✅ | ✅ | ✅ | ✅ | ✅ |
| 查找替换（正则/大小写） | ✅ | ✅ | ✅ | ✅ | ✅ |
| 自定义快捷键 | ✅ | ❌ | ✅（插件） | ✅ | ❌ |

---

## 文件与导航

| 功能 | MarkLite | Typora | Obsidian | VS Code | Mark Text |
|------|----------|--------|----------|---------|-----------|
| 文件树侧边栏 | ✅ | ✅ | ✅ | ✅ | ✅ |
| 文件树 CRUD（新建/删除/重命名） | ✅ | ✅ | ✅ | ✅ | ❌ |
| 大纲 / TOC 导航 | ✅ | ✅ | ✅ | ✅（插件） | ✅ |
| 跨文件全文搜索 | ✅ | ❌ | ✅ | ✅ | ❌ |
| Wiki-Link 导航（`[[target]]`） | ✅ | ❌ | ✅（核心） | ❌ | ❌ |
| 最近文件历史 | ✅ | ✅ | ✅ | ✅ | ✅ |
| 命令面板（Ctrl+Shift+P） | ✅ | ❌ | ✅ | ✅ | ❌ |

---

## 内容增强

| 功能 | MarkLite | Typora | Obsidian | VS Code | Mark Text |
|------|----------|--------|----------|---------|-----------|
| GFM（表格/任务列表/删除线） | ✅ | ✅ | ✅ | ✅（插件） | ✅ |
| 数学公式（KaTeX） | ✅ | ✅ | ✅ | ✅（插件） | ✅ |
| Mermaid 图表 | ✅ | ✅ | ✅（插件） | ✅（插件） | ✅ |
| 脚注（`[^1]`） | ✅ | ✅ | ✅ | ✅（插件） | ✅ |
| YAML Frontmatter（高亮+预览） | ✅ | ✅ | ✅ | ✅ | ✅ |
| 代码高亮（100+ 语言） | ✅（highlight.js） | ✅ | ✅ | ✅ | ✅ |
| 表格可视化编辑 | ✅ | ✅（原地） | ✅（插件） | ✅（插件） | ❌ |
| 自定义容器（`:::info`） | ✅ | ❌ | ❌ | ✅（插件） | ❌ |

---

## 导出格式

| 格式 | MarkLite | Typora | Obsidian | VS Code | Mark Text |
|------|----------|--------|----------|---------|-----------|
| PDF | ✅ | ✅ | ✅（付费插件） | ✅（插件） | ✅ |
| DOCX（Word） | ✅ | ✅ | ✅（付费插件） | ✅（插件） | ❌ |
| HTML（独立文件） | ✅ | ✅ | ✅ | ✅ | ✅ |
| EPUB | ✅ | ❌ | ✅（插件） | ✅（插件） | ❌ |
| PNG（截图） | ✅ | ❌ | ❌ | ❌ | ❌ |

---

## 辅助工具

| 功能 | MarkLite | Typora | Obsidian | VS Code | Mark Text |
|------|----------|--------|----------|---------|-----------|
| 片段管理器（变量替换） | ✅ | ❌ | ✅（插件） | ✅（插件） | ❌ |
| 本地版本快照（最多 20 条） | ✅ | ❌ | ✅（插件） | ✅（内置） | ❌ |
| 图片粘贴/拖拽（自动保存） | ✅ | ✅ | ✅ | ✅（插件） | ✅ |
| 写作统计（字数/句数/阅读时间） | ✅ | ✅ | ✅（插件） | ✅（插件） | ✅ |
| 拼写检查 | ✅（浏览器原生） | ✅ | ✅（插件） | ✅（插件） | ✅ |
| 内置用户手册 | ✅（Toolbar `?` 按钮） | ❌ | ✅（Help 页面） | ✅（文档站） | ❌ |

---

## 界面与体验

| 功能 | MarkLite | Typora | Obsidian | VS Code | Mark Text |
|------|----------|--------|----------|---------|-----------|
| 主题数量 | 4（Light/Dark/Sepia/HC） | ~10 | 数十（社区） | 数百（扩展） | ~5 |
| 跟随系统深色模式 | ✅ | ✅ | ✅ | ✅ | ✅ |
| 打字机模式 | ✅ | ✅ | ✅（插件） | ❌ | ✅ |
| 专注模式 | ✅ | ✅ | ✅（禅模式） | ❌ | ✅ |
| 全屏模式 | ✅ | ✅ | ✅ | ✅ | ✅ |
| 中文界面（i18n） | ✅（中/英双语） | ✅ | ✅ | ✅ | ✅ |
| 编辑↔预览同步滚动 | ✅ | N/A | ✅ | ✅（插件） | ✅ |
| 欢迎页 / 快速入口 | ✅ | ❌ | ✅ | ❌ | ❌ |

---

## 平台支持

| 平台 | MarkLite | Typora | Obsidian | VS Code | Mark Text |
|------|----------|--------|----------|---------|-----------|
| Windows | ✅ | ✅ | ✅ | ✅ | ✅ |
| macOS | ✅ | ✅ | ✅ | ✅ | ✅ |
| Linux | ✅ | ✅ | ✅ | ✅ | ✅ |
| 移动端（iOS/Android） | ❌ | ❌ | ✅（付费） | ❌ | ❌ |
| Web 版 | ❌ | ❌ | ✅（付费） | ✅（vscode.dev） | ❌ |

---

## 场景推荐

| 使用场景 | 推荐工具 | 理由 |
|----------|----------|------|
| 轻量日常写作 | **MarkLite** | 快速启动，功能完备，完全免费开源 |
| WYSIWYG 所见即所得 | Typora | 最佳渲染体验，编辑即预览 |
| 个人知识库 / PKM | Obsidian | 双链笔记、知识图谱等高级功能 |
| 技术文档 / 代码结合 | VS Code | 与开发工作流深度集成 |
| 简洁开源编辑 | Mark Text | 干净界面，支持格式完整 |

> **MarkLite 最适合**：需要多标签页管理、多光标编辑、EPUB 导出、Wiki-Link 导航、版本快照或片段管理的用户，同时希望工具完全免费且开源。
